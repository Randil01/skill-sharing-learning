...
const validationSchema = Yup.object().shape({
  ...
  mediaUrl: Yup.string().nullable().required("You need to upload an image or video"),
});
...
const UploadPost = () => {
  ...
  const formik = useFormik({
    initialValues: {
      content: "",
      mediaUrl: null, // Store URL instead of file
    },
    validationSchema,
  ...
  const uploadToCloudinary = async (file) => {
    ...
    const res = await fetch("https://api.cloudinary.com/v1_1/dhxfhjo5r/auto/upload", {
      method: "POST",
      body: data,
    });
      ...
      await new Promise((resolve) => {
        videoElement.onloadedmetadata = () => {
          if (videoElement.duration > 30) {
            alert("Video duration cannot be longer than 30 seconds");
          ...
          <div className="w-full">
            <form onSubmit={formik.handleSubmit}>
              ...
              {selectedMediaPreview && (
                <div className="mt-3">
                  {mediaType === "image" ? (
                    <img src={selectedMediaPreview} alt="Preview" className="w-48 rounded-md border" />
                  ) : (
                    <video src={selectedMediaPreview} controls className="w-48 rounded-md border" />
                  )}
                </div>
              )}
              ...
              {formik.errors.mediaUrl && formik.touched.mediaUrl && (
